...
RSpec.describe Event do
  ...
  describe "(avoiding duplicates, in the same cohort)" do
    it "does NOT allow events within #{Event.duplicate_date_delta / 1.minute} minutes of each other" do
      first_event = cohort.events.create!(occurred_at: (Event.duplicate_date_delta - 1).ago, title: "FIRST")
      too_soon = cohort.events.build(occurred_at: Time.now, title: "TOO SOON")
      expect(too_soon).to_not be_valid
      expect(too_soon.errors[:occurred_at]).to include(/occurs too close/)
    end

    it "does allow another event, after #{Event.duplicate_date_delta / 1.minute} minutes" do
      first_event = cohort.events.create!(occurred_at: (Event.duplicate_date_delta + 1).ago, title: "FIRST")
      enough_delay = cohort.events.build(occurred_at: Time.now, title: "ENOUGH DELAY")
